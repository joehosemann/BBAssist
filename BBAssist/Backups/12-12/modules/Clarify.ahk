...
global
	...
	{
		IniRead, SiteID, %CachePath%, %CaseId%, SiteID
		
		if (clarifySiteID := "ERROR")
		{
			ClarifySetControls()
		}
		else
...
global
	...
	Loop, parse, tempvalue, `n, `r
	{
		if % controlIndex[ A_Index ] != "" ;%
		{
			...
			ClarifyGetCtrlText(controlIndex[ A_Index ])				
		}
		if %A_Index% > 80
			break
	}   
...
; x = varname
ClarifyGetCtrlText(x)    
{
global


	Logger("ClarifyGetCtrlText",3)
	if x = ContactFullName
	{	
		ControlGetText, tempvalue, , ahk_id %hwndContactFullName%
		
		ContactFullName := tempvalue
	   
		StringSplit, tempvalue, tempvalue, %A_Space%
		ContactFirstName = %tempvalue1%
		ContactLastName = %tempvalue2%
		
		tempvalue := ""
	}
	else
	{		
		ControlGetText, %CaseId%%x%, , % "ahk_id" . hwnd%x%	
	}	
	return
}
...
global
	...
	;if PreviousWindowType != %WindowType%
	;{
		; Delete old backups
